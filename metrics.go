...
func (m *Metrics) Record(ctx context.Context, caller string, key KeyInterface, duration time.Duration, success bool) {
	for _, metric := range m.metrics {
		metric.Record(ctx, caller, key, duration, success)
	}
}

func (m *Metrics) RecordMultiple(ctx context.Context, caller string, key []KeyInterface, duration time.Duration, success bool) {
	for _, key := range key {
		m.Record(ctx, caller, key, duration, success)
	}
}
